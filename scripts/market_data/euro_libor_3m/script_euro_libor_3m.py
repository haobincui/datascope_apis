...
for idx, ele in enumerate(years):
    for month in months:
        ...
        if query_end_date.date() > date.today():
            query_end_date = datetime(2023, 2, 9, 0, 0, 0)
        s = time.time()
        res = get_intraday_data(identifier, required_fields, query_start_date, query_end_date, multi_thread=True)
        os.makedirs(f'./{identifier}/{ele}/', exist_ok=True)
        if isinstance(res, ExtractionError):
            logging.warning(f'Extraction failed: [{res.message}]')
            continue
        res.to_csv(f'./{identifier}/{ele}/{identifier}-{query_start_date.date().isoformat()}-{query_end_date.date().isoformat()}.csv')
        e = time.time()
